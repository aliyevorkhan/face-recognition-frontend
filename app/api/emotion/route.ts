...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("Received emotion request with API key:", apiKey ? `${apiKey.substring(0, 4)}...` : "missing")
    ...
    console.log("Forwarding request to emotion API...")
    ...
    try {
      ...
      console.log("Emotion API response headers:", Object.fromEntries(response.headers.entries()))

      // Get response as text first for debugging
      const responseText = await response.text()
      console.log(
        "Emotion API response body:",
