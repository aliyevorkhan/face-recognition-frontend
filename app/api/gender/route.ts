...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("Received gender request with API key:", apiKey ? `${apiKey.substring(0, 4)}...` : "missing")
    ...
    console.log("Forwarding request to gender API...")
    ...
    try {
      ...
      // Note: Changed the request body format to match expected API format
      const requestBody = JSON.stringify({ image: imageBase64 })
      console.log("Request body length:", requestBody.length)

      const response = await fetch(GENDER_API_URL, {
        method: "POST",
        headers: {
      ...
      console.log("Gender API response status:", response.status)
      console.log("Gender API response headers:", Object.fromEntries(response.headers.entries()))

      // Get response as text first for debugging
      const responseText = await response.text()
      console.log(
        "Gender API response body:",
    ...
    console.error("Gender API proxy error:", error)
